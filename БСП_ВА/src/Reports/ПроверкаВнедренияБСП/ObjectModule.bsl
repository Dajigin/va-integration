...
Процедура Подключаемый_СтруктураПодчиненности_ПроверитьВнедрение()
	...
	// Если в конфигурацию встроена подсистема ПодключаемыеКоманды, рекомендуется очистить
	// типы параметров общей команды СвязанныеДокументы, так как подсистема сама определит объекты,
	// в подменю Отчеты которых будет выводиться команда Связанные документы.
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПодключаемыеКоманды") Тогда
		СоставКоманды = СоставТипаИзСтроки("ОбщиеКоманды.СвязанныеДокументы.ТипПараметраКоманды");
		Если СоставКоманды.Количество() > 0 Тогда 
			ПредставлениеОбъекта = ПредставлениеТипОбъектаМетаданных(Метаданные.ОбщиеКоманды["СвязанныеДокументы"]);
			ПодробноеОписаниеОшибки = НСтр("ru = 'В конфигурацию встроена подсистема ПодключаемыеКоманды,
				|рекомендуется очистить типы параметров общей команды СвязанныеДокументы,
				|так как подсистема сама определит объекты,
				|в подменю Отчеты которых будет выводиться команда Связанные документы'");
			ДобавитьОшибку(ПредставлениеОбъекта, НСтр("ru = 'Состав типов не очищен'"), ПодробноеОписаниеОшибки);
		КонецЕсли;
