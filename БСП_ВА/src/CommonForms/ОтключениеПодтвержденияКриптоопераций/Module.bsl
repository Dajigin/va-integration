...
Процедура Подтвердить(Команда)
	...
	Если ШагАвторизации > 2 И ПарольПроверен Тогда
		...
		ПараметрыФормы = Новый Структура("СпособПодтвержденияКриптоопераций, Выполнено", ПредопределенноеЗначение("Перечисление.СпособыПодтвержденияКриптоопераций.ДолговременныйТокен"), Истина);
...
Процедура ПроверитьКодТелефона(ТекущийКод)
	...
	ИначеЕсли ВыполняетсяПроверка Тогда
		СтатусКод 	= НСтр("ru = 'Ожидается ответ'");
		ОтключитьОбратныйОтсчет = 10;
		
	ИначеЕсли ОтключитьОбратныйОтсчет = 99 Тогда
		СтатусКод 	= НСтр("ru = 'Необходимо повторить процедуру'");
	...
	Иначе
		СтатусКод 	= НСтр("ru = 'Пароль не указан'");
		ОтключитьОбратныйОтсчет = 10;
...
Процедура ИзменитьВидимостьПароля(ПринудительноСпрятать = Ложь)
	...
	Если ПринудительноСпрятать Тогда
		...
		Элементы.ПарольПользователя.ТолькоПросмотр = Истина;
	...
	ИначеЕсли Не НовыйРежим Тогда
		...
		Элементы.ПарольПользователя.ТолькоПросмотр = Ложь;
