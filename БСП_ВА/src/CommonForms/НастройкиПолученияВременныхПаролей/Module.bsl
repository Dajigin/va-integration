...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаЗаголовкаИКодаСтарогоНомера.Видимость = Форма.ПроверкаТелефонСтарый.ВыполняетсяПроверка 
							И МожноПодтвердитьИзменения 
							И Форма.РежимПроверки = "Изменение";
...
&НаКлиентеНаСервереБезКонтекста
Функция ПроверкаТелефонДляПаролей(ЗначениеВведено, ВыполняетсяПроверка, ИдентификаторПроверки, ПодтверждениеВыполнено, КодОтправлен, ИсходноеЗначение)
