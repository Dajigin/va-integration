	...
	Если ЭтоИзменениеЗаголовка Тогда
		...
		Если ЗначениеРеквизитаСтруктуры = Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У объекта метаданных нет реквизита ""%1"" %2'"), ИмяРеквизита,
				?(ПустаяСтрока(ИмяРеквизитаТЧ), "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'табличной части ""%1""'"), ИмяРеквизитаТЧ)));
		ИначеЕсли ЗначениеРеквизитаСтруктуры.Исключаемый Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Реквизит ""%1"" %2 исключаемого типа %3'"), ИмяРеквизита,
				?(ПустаяСтрока(ИмяРеквизитаТЧ), "",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'табличной части ""%1""'"), ИмяРеквизитаТЧ)), ЗначениеРеквизитаСтруктуры.ТипЗначения);
...
Процедура ЗаполнитьДеревоРеквизитов()
	...
	Для каждого Реквизит Из ПолнаяСтруктураОбъекта Цикл
		...
		Для каждого РеквизитТЧ Из Реквизит.Значение.Реквизиты Цикл
			...
			НоваяСтрокаТЧ.Локализуемый = Реквизит.Значение.Локализуемый;
...
Функция ЗаполнитьОбщиеЯзыковыеПараметры()
	...
	Для Каждого Язык Из Метаданные.Языки Цикл
		...
		СтрокаЯзык.Представление = СтрокаЯзык.Имя + " (" + СтрокаЯзык.КодЯзыка + ")";
...
Функция ИнициализироватьТабличныеДокументы(ДанныеИнициализации = Неопределено)
	...
	Для каждого Реквизит Из РеквизитыИнициализации Цикл
		...
		ИначеЕсли Реквизит.Имя = "ИмяПредопределенныхДанных" Тогда
			Область.Текст = НСтр("ru = 'Ключ ссылки'");
		Иначе
...
Процедура ПреобразоватьВКодНаСервере()
	...
	Код1С.ДобавитьСтроку("Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы) Экспорт");
...
Процедура ПреобразоватьТабличныеДанныеВКод(ИсточникДанных, ДанныеИнициализации = Неопределено, ИмяТабличнойЧасти = "")
	...
	СтрокаИменПредопределенныхДанных = ""; //для проверки уникальности Реквизита "ИмяПредопределенныхДанных"
	РеквизитСтруктуры = Неопределено;
	...
	Для каждого СтрокаДанных Из ИсточникДанных Цикл
		...
		Если ПустаяСтрока(ИмяТабличнойЧасти) И ВыводитьИменПредопределенныхДанных Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяПредопределенныхДанных) Тогда
				Продолжить;
			ИначеЕсли СтрНайти(СтрокаИменПредопределенныхДанных, ИмяПредопределенныхДанных + ",") > 0 Тогда
				// строка с таким ИменемПредопределенныхДанных уже была
				Продолжить;
			Иначе
				СтрокаИменПредопределенныхДанных = СтрокаИменПредопределенныхДанных + ИмяПредопределенныхДанных + ",";
			КонецЕсли;
		...
		Для каждого Реквизит Из РеквизитыИнициализации Цикл
			...
			ИначеЕсли ЭтоОписаниеТипов(ТипРеквизита) Тогда
				...
				Для каждого Тип Из ЗначениеРеквизита.Типы() Цикл
					...
					Если ОбъектМетаданныхПоТипу <> Неопределено Тогда
						Код1С.ДобавитьСтроку(Символы.Таб 
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("МассивТипов.Добавить(Тип(%1));",ТекстТипа));
					КонецЕсли;
				КонецЦикла;
			...
			ИначеЕсли ЭтоПростойТип И ОписаниеТиповСтрока.СодержитТип(ТипРеквизита)
				...
				Если ИмяРеквизита <> "ИмяПредопределенныхДанных" Тогда
					...
					ЗначениеРеквизитаСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("НСтр(""%1 = '%2'"")", ОбщиеЯзыковыеПараметры.ОсновнойЯзык, ЗначениеРеквизитаСтрока);
				...
				Код1С.ДобавитьСтроку(Символы.Таб 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;", Элемент, ИмяРеквизита, ЗначениеРеквизитаСтрока));
			...
			ИначеЕсли ЭтоПростойТип И ОписаниеТиповСтрока.СодержитТип(ТипРеквизита) Тогда
				...
				Для каждого ЯзыкЛокализации Из Объект.Языки Цикл
					...
					Если ЗначениеЗаполнено(ЗначениеРеквизитаЛ) Тогда
						ТекстНСтр = ТекстНСтр 
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1" + ПереносСтроки + "|%2 = '%3'", Разделитель, ЯзыкЛокализации.КодЯзыка, ЗначениеРеквизитаСтрока);
						Разделитель = ";";
					КонецЕсли;
				...
				Код1С.ДобавитьСтроку(Символы.Таб 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Элемент, ""%1"", %2, КодыЯзыков); // @ НСтр", ИмяРеквизита, ТекстНСтр));
				
			ИначеЕсли ЭтоПростойТип И ОписаниеТиповБулево.СодержитТип(ТипРеквизита) Тогда
				Код1С.ДобавитьСтроку(Символы.Таб 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;", Элемент, ИмяРеквизита, ?(ЗначениеРеквизита, "Истина", "Ложь")));
			ИначеЕсли ЭтоПростойТип И ОписаниеТиповДата.СодержитТип(ТипРеквизита) И ЗначениеЗаполнено(ЗначениеРеквизита) Тогда
				Код1С.ДобавитьСтроку(Символы.Таб 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;", Элемент, ИмяРеквизита, "Дата(""" + Формат(ЗначениеРеквизита, "ДФ=yyyyMMddЧЧммсс") + """)"));
			ИначеЕсли ЭтоПростойТип И ОписаниеТиповЧисло.СодержитТип(ТипРеквизита) Тогда
				Код1С.ДобавитьСтроку(Символы.Таб 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;", Элемент, ИмяРеквизита, Формат(ЗначениеРеквизита, "ЧРД=.; ЧН=0; ЧГ=")));
			ИначеЕсли ЭтоПростойТип И БизнесПроцессы.ТипВсеСсылкиТочекМаршрутаБизнесПроцессов().СодержитТип(ТипРеквизита) Тогда
				...
				Код1С.ДобавитьСтроку(Символы.Таб 
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3; // Представление ссылки ТочкаМаршрутаБизнесПроцесса", Элемент, ИмяРеквизита, ЗначениеРеквизитаСтрока));
			Иначе//Если ЭтоСсылочныйРеквизит Тогда
				...
				Если Метаданные.НайтиПоПолномуИмени("ПланОбмена." + МетаданныеРеквизита.Имя) <> Неопределено Тогда
					...
					КонецЕсли;
					Код1С.ДобавитьСтроку(Символы.Таб 
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = ПланыОбмена[""%3""].%4;", Элемент, ИмяРеквизита, МетаданныеРеквизита.Имя, ТекстПоиска));
				...
				ИначеЕсли Метаданные.НайтиПоПолномуИмени("Справочник." + МетаданныеРеквизита.Имя) <> Неопределено
					...
					ИначеЕсли ВариантыВводаПоСтроке.Свойство("Код", ТипКода) Тогда
						...
						Если МенеджерыСсылок.Свойство(СтрЗаменить(МетаданныеРеквизитаПолноеИмя, "." + МетаданныеРеквизита.Имя, ""), МенеджерОбъекта) Тогда
							...
							Код1С.ДобавитьСтроку(Символы.Таб 
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("НайденнаяСсылка = %1.НайтиПоКоду(%2);", (МенеджерОбъекта + МетаданныеРеквизита.Имя), ЗначениеКода));
							Код1С.ДобавитьСтроку(Символы.Таб 
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;", Элемент, ИмяРеквизита, "НайденнаяСсылка"));
						КонецЕсли;
					...
					ИначеЕсли ВариантыВводаПоСтроке.Свойство("Наименование") Тогда
						...
						Если МенеджерыСсылок.Свойство(СтрЗаменить(МетаданныеРеквизитаПолноеИмя, "." + МетаданныеРеквизита.Имя, ""), МенеджерОбъекта) Тогда
							...
							Код1С.ДобавитьСтроку(Символы.Таб 
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("НайденнаяСсылка = %1.НайтиПоНаименованию(%2);", (МенеджерОбъекта + МетаданныеРеквизита.Имя), ЗначениеРеквизитаСтрока));
							Код1С.ДобавитьСтроку(Символы.Таб 
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3;", Элемент, ИмяРеквизита, "НайденнаяСсылка"));
						КонецЕсли;
					...
					Иначе// поиск по реквизиту
						ЗначениеРеквизитаСтрока = ЗначениеСтроковогоРеквизитаДляКода(ЗначениеРеквизита, Ложь);
						Код1С.ДобавитьСтроку(Символы.Таб 
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3; // Представление %4", Элемент, ИмяРеквизита, ЗначениеРеквизитаСтрока, МетаданныеРеквизитаПолноеИмя));
				...
				Иначе
					ЗначениеРеквизитаСтрока = ЗначениеСтроковогоРеквизитаДляКода(ЗначениеРеквизита, Ложь);
					Код1С.ДобавитьСтроку(Символы.Таб 
						+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1.%2 = %3; // Представление ссылки", Элемент, ИмяРеквизита, ЗначениеРеквизитаСтрока));
