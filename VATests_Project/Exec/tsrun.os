...
Функция ОбщиеПараметрыВыполнения(ПараметрыКоманднойСтроки)
	...
	ПараметрыВыполнения.Вставить("ИсходныйКаталогДистрибутивов", ПараметрыВыполнения.БазовыйКаталогТестов + "\Exec");

	ИмяГруппыНабора = ПараметрыВыполнения.ИмяГруппыНабора;

	ПараметрВыходнойКаталог = ПараметрыКоманднойСтроки.Получить("outdir");
	Если ПараметрВыходнойКаталог = Неопределено Тогда
		КаталогВФ = КаталогВременныхФайлов();
	Иначе
		КаталогВФ = ПараметрВыходнойКаталог + "\";
	КонецЕсли;

	КаталогОтчетов  = ПараметрыКоманднойСтроки.Получить("reportsdir");

	Если КаталогОтчетов <> Неопределено Тогда
		ПараметрыВыполнения.Вставить("КаталогОтчетов", КаталогОтчетов);
	КонецЕсли;
...
Функция НастройкиНабораТестов(ФайлНабора, ОбщиеПараметрыВыполнения)
	...
	КаталогРезультатовТестов = Неопределено;
	ОбщиеПараметрыВыполнения.Свойство("КаталогОтчетов", КаталогРезультатовТестов);
	Если  КаталогРезультатовТестов = Неопределено Тогда
...
Процедура СформироватьСписокТестов(ПараметрыВыполнения)
	СообщениеЛогирования("Формирование списка тестов:", УровеньЛогирования_Сокращенный());
	...
	СообщениеЛогирования("Формирование списка тестов завершено", УровеньЛогирования_Сокращенный());
КонецПроцедуры
...
Процедура ВыполнитьТестыВА(НастройкиНабора, ОбщиеПараметрыВыполнения)
	ИсходныйКаталогДистрибутивов = ОбщиеПараметрыВыполнения.ИсходныйКаталогДистрибутивов;
	...
	ПутьКОбработкеВА = ИсходныйКаталогДистрибутивов + "\vanessa-automation-single.epf";

	

	ИмяПользователя = ПараметрыКоманднойСтроки.Получить("dbuser");
	Пароль = ПараметрыКоманднойСтроки.Получить("pass");
...
// Параметры:
// ОписаниеНабораГруппы - Структура - описание набора тестов или группы наборов (см.ЭлементГруппыНаборов())
// ОбщиеПараметрыВыполнения - Структура - общие параметры выполнения тестов (см. ОбщиеПараметрыВыполнения())
Процедура СгенерироватьОтчетыАллюр(ОписаниеНабораГруппы, ОбщиеПараметрыВыполнения)
	...
	ПутьКИсточникуДистрибутива = ИсходныйКаталогДистрибутивов + "\allure.zip";
	...
	ОткрытьОтчетАллюрНабораГруппы(ОписаниеНабораГруппы, КаталогДистрибутива);

КонецПроцедуры

Процедура ОткрытьОтчетАллюрНабораГруппы(ЭлементНабораГруппы, КаталогДистрибутиваАллюр)
	Если ЭлементНабораГруппы.ЭтоВложеннаяГруппа Тогда
		Для Каждого ПодчиненныйЭлемент Из ЭлементНабораГруппы.ЭлементыГруппы Цикл
			ОткрытьОтчетАллюрНабораГруппы(ПодчиненныйЭлемент, КаталогДистрибутиваАллюр);
		КонецЦикла;
	Иначе
		ПутьИсходныхФайловОтчета = ЭлементНабораГруппы.НастройкиНабора.ПутьКОтчетамAllure;	
...
// Выполняет тесты группы наборов тестов или набора тестов.
// Параметры:
// ГруппаНабор - Структура - описание набора тестов или группы наборов тестов. См. ЭлементГруппыНаборов().
// ОбщиеПараметрыВыполнения - Структура - общие параметры выполнения тестов (см. ОбщиеПараметрыВыполнения())
Процедура ВыполнитьТестыГруппыНабора(ГруппаНабор, ОбщиеПараметрыВыполнения)

	Если ГруппаНабор.ЭтоВложеннаяГруппа Тогда
		Для Каждого ЭлементГруппы Из ГруппаНабор.ЭлементыГруппы Цикл 
			ВыполнитьТестыГруппыНабора(ЭлементГруппы, ОбщиеПараметрыВыполнения);
		КонецЦикла;
	Иначе
	 	ВывестиРазделительЛогирования();
	 	СообщениеЛогирования("Выполняется набор: " + ГруппаНабор.Имя, УровеньЛогирования_Сокращенный());
	 	ВыполнитьТесты(ГруппаНабор.НастройкиНабора, ОбщиеПараметрыВыполнения);
	КонецЕсли;
...
Функция ИнициализироватьФайлПараметров(НастройкиНабора, ОбщиеПараметрыВыполнения)
	...
	ИмяФайлаИсточника = ИсходныйКаталогДистрибутивов + "\VAParams_Tmplt.json";
...
Функция ОписаниеКаталогаНаборовГруппы(КаталогГруппы, ОбщиеПараметрыВыполнения)
	...
	Для Каждого ЭлементКаталога Из ЭлементыКаталога Цикл
		Если ЭлементКаталога.ЭтоКаталог() Тогда
			...
			Результат.ЭлементыГруппы.Добавить(ОписаниеКаталогаНаборовГруппы(ЭлементКаталога, ОбщиеПараметрыВыполнения));
		Иначе
...
Процедура ВывестиПодсказкуЗапуска()
	Сообщить("tsrun v. 1.0.0. Запуск тестов из командной строки. Требуется наличие OneScript.");
	Сообщить("Используется для запуска тестов VA и вывода результата тестирования в рамках проекта интеграции Vanessa Automation с конфигурациями на базе БСП");
	Сообщить("адрес проекта: https://github.com/Dajigin/va-integration");
	Сообщить(Символы.ПС);
	Сообщить("Строка запуска: oscript tsrun.os [--Option1 [Value1]] [--Option2 [Value2]]  ....");
	Сообщить(Символы.ПС);
	Сообщить("Описание параметров:");
	Сообщить(Символы.ПС);

	ОписанияПараметров = Новый Массив();
	ДобавитьОписаниеПараметра(ОписанияПараметров, "testspath", "(обязательный) базовый каталог проекта тестов (VATests_Project)");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "testsetname", "(обязательный) относительный (относительно каталога НаборыТестов) путь к файлу набора или каталогу группы наборов тестов");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "ibconnection", "(обязательный) путь к базе для тестирования (поддерживается только файловый вариант)");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "v8version", "(обязательный) полная версия (с точностью до номера сборки) платформы ""1С:Предприятие""");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "dbuser", "имя пользователя ");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "pass", "пароль пользователя");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "outdir", "временный каталог, в фотором формируются тесты для выполнения. Если не указан - создается временный каталог в профиле.");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "reportsdir", "каталог отчетов о выполнении тестов. Если не указан - формируются во временном каталоге тестов.");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "help", "(без значения) вывод справки ");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "run", "(без значения) выполнить тесты");
	ДобавитьОписаниеПараметра(ОписанияПараметров, "allure_show", "(без значения) сформировать и открыть отчет Allure после завершения выполнения тестов");
	Для Каждого ОписаниеПараметра Из ОписанияПараметров Цикл
		Разделитель = Символы.Таб + Символы.Таб;
		Сообщить(ОписаниеПараметра.ИмяПараметра + Разделитель + " - " + ОписаниеПараметра.ОписаниеПараметра);
	КонецЦикла;

	Сообщить(Символы.ПС);
	Сообщить("Примеры:");
	Сообщить("см. tsrun_sample_set.cmd - для указанного набора тестов");
	Сообщить("см. tsrun_sample_group.cmd - для группы наборов тестов");

КонецПроцедуры

Процедура ДобавитьОписаниеПараметра(МассивПараметров, ИмяПараметра, ОписаниеПараметра)
	ОписаниеПараметра = Новый Структура("ИмяПараметра,ОписаниеПараметра", ИмяПараметра, ОписаниеПараметра);
	МассивПараметров.Добавить(ОписаниеПараметра);
КонецПроцедуры

ПараметрыКоманднойСтроки = ПараметрыКоманднойСтрокиВСоответствие();
Если ПараметрыКоманднойСтроки.Получить("help")  <> Неопределено 
		Или ПараметрыКоманднойСтроки.Количество() = 0 Тогда
		ВывестиПодсказкуЗапуска();
		ЗавершитьРаботу(0);
КонецЕсли;
...
ПутьГруппыИлиФайла = ОбщиеПараметрыВыполнения.БазовыйКаталогТестов + "\НаборыТестов\" + ОбщиеПараметрыВыполнения.ИмяГруппыНабора;
КаталогГруппыИлиФайлНабора = Новый Файл(ПутьГруппыИлиФайла);
Если КаталогГруппыИлиФайлНабора.ЭтоКаталог() Тогда
	ОписаниеНабораГруппы = ОписаниеКаталогаНаборовГруппы(КаталогГруппыИлиФайлНабора, ОбщиеПараметрыВыполнения);
Иначе
	ОписаниеНабораГруппы = ЭлементГруппыНаборов();
	ОписаниеНабораГруппы.Имя = КаталогГруппыИлиФайлНабора.Имя;
	ОписаниеНабораГруппы.ЭтоВложеннаяГруппа = Ложь;
	ОписаниеНабораГруппы.НастройкиНабора = НастройкиНабораТестов(КаталогГруппыИлиФайлНабора, ОбщиеПараметрыВыполнения);
КонецЕсли;
...
СформироватьФайлыНаборовДляВыполнения(ОписаниеНабораГруппы, ОбщиеПараметрыВыполнения);
Если ПараметрыКоманднойСтроки.Получить("run") <> Неопределено Тогда
	ВыполнитьТестыГруппыНабора(ОписаниеНабораГруппы, ОбщиеПараметрыВыполнения);
	Если ПараметрыКоманднойСтроки.Получить("allure_show") <> Неопределено Тогда
		СгенерироватьОтчетыАллюр(ОписаниеНабораГруппы, ОбщиеПараметрыВыполнения);
	КонецЕсли;
КонецЕсли;