...
Процедура СформироватьСписокТестов()
	...
	ПутьКФайлуИнициализации = БазовыйКаталогТестов + "\" + ИмяФайлаОписанияТестов();
...
Процедура СформироватьФайлыТестовДляВыполнения(НастройкиНабора)
...
КонецФункции
// Возвращает параметры для формирования файла VAParams.json
// Результат:
...
// * Значение - Строка - значение, которое будет подставлено вместо параметра.
Функция ПараметрыВыполнения()
	...
	СоздатьКаталогПриОтсутствии(КаталогНабора, Ложь);
	...
	ПараметрыВыполнения.Вставить("ВерсияПлатформы", ВерсияПлатформы);
...
РазобратьПараметрыЗапуска("testspath,testsetname,ibconnection,v8version");
СоздатьВременныеКаталоги();
Тесты = ТаблицаТестов();
СформироватьСписокТестов();
ПутьКФайлуНабора = БазовыйКаталогТестов + "\НаборыТестов\" + ИмяНабораТестов + ".tsc";
НастройкиНабора = ЗагрузитьНастройкиНабора(ПутьКФайлуНабора);
СформироватьФайлыТестовДляВыполнения(НастройкиНабора);
