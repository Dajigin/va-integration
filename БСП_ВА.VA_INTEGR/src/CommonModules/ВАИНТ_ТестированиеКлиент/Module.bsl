...
Процедура ДополнитьПараметрыЗапускаКлиентов(ФормаВА) Экспорт
	...
	ПараметрАвтоТестирования = "АвтоТестыВыполняются";
	Для Каждого СтрокаКлиента Из ФормаВА.ДанныеКлиентовТестирования Цикл
		Если ПараметрыАвтоТестирования.ЛогироватьОбъектыАвтоТестирования Тогда
			Если СтрНайти(СтрокаКлиента.ДопПараметры, ПараметрАвтоТестирования) <> 0 Тогда
				Продолжить;
			КонецЕсли;
			...
			КонецЕсли;
		Иначе
			СтрокаКлиента.ДопПараметры = СтрЗаменить(СтрокаКлиента.ДопПараметры, ПараметрАвтоТестирования, "");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
#КонецОбласти
