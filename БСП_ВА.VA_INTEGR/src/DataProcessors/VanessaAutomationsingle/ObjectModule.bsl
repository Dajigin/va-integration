...
Функция СтруктураОбщихНастроек(НастрокиВыгрузки = Ложь, НастройкиЗагрузки = Ложь) Экспорт 
	...
	Настройки.Вставить("ИспользоватьДанныеТекущейФормыПриПодбореШагов", Истина);
	...
	Настройки.Вставить("ВычислятьВыраженияВФигурныхСкобках", Истина);
...
Функция ВставитьГурппыШагов(СтрокаВыполнения, ДопПараметрыАллюр2, РезультатыПрохождения, МассивШагов)
	...
	МассивШагов = ВставитьГурппыШагов(СтрокаВыполнения.Родитель, ДопПараметрыАллюр2, РезультатыПрохождения, МассивШагов);
...
Процедура ЗаписатьВыполнениеШагаАллюр2(МассивШагов, ДеревоТестов, РезультатыПрохождения, СоответствиеГрупп, МассивXMLОтчетаAllure, СтруктураВыгрузкиСценария, ДопПараметрыАллюр2)
	...
	Если СтрокаВыполнения <> Неопределено Тогда
		МассивШаговДобавления = ВставитьГурппыШагов(СтрокаВыполнения.Родитель, ДопПараметрыАллюр2, РезультатыПрохождения, МассивШаговДобавления);
	КонецЕсли;
...
Процедура РассчитатьStepDefinition(Параметры)
	...
	СтарыйStepDefinition = ПолучитьStepDefinitionПоСтроке(Параметры.ИмяШагаБезКлючевогоСлова,
															...
															Параметры.КоличествоПередаваемыхТаблиц,,СтруктураПараметров, МногострочныеПараметрыШага);
		...
		Если Лев(Стр, 3) = """""""" Тогда
			...
			Если НЕ ИдетСчитываниеМногострочногоПараметра Тогда
				Если ИдетЧтениеСекцииПеременные Тогда
					...
					//ДанныеПеременной.Вставить("НомерСтроки", СчетчикСтрокФичи);
					...
					//СтруктураПараметров.ДанныеСекцииПеременные.ПростыеПеременныеКакВФайле.Добавить(ИмяМногоСтрочнойПеременной);
				Иначе	
...
Функция ПолучитьStepDefinitionПоСтроке(Знач Стр, ЗначенияПараметров, СтрокаПараметров = "",
								...
								ДанныеОбработкиПараметров = Неопределено, СтруктураПараметров = Неопределено, МногострочныеПараметрыШага = Неопределено) 
	...
	Для Каждого ДанныеСтроки Из МассивСтрок Цикл
		...
		Если Поз > 0 Тогда
			...
			Иначе
				...
				ЗначениеПеременной = УбратьОбрамляющиеСимволыИзСтроки(ЗначениеПеременной);
				ДанныеПеременной = Новый Структура;
		...
		ИначеЕсли Лев(Стр, 1) = "|" Тогда
			...
			Иначе
				...
				Для Сч = 0 По Данные.Количество() - 1 Цикл
					ДанныеСтроки.Вставить(ДанныеТаблицы.Колонки[Сч], Данные[Сч].Значение);
					Если ЗначениеЗаполнено(ИмяФайлаФичиИмпорт) Тогда
