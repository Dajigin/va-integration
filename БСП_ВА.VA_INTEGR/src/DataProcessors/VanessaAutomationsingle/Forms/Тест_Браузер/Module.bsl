...
Функция ЯПоказываюПодсказкуEnjoyHintПоКоординатамВБраузере(Текст, ИмяПеременной = Неопределено, ТабПарам = Неопределено) Экспорт
	...
	Скрипт = СтрЗаменить(Скрипт, "%4", XMLСтрока(Цел(ДанныеПараметров["left"])));
	Скрипт = СтрЗаменить(Скрипт, "%5", XMLСтрока(Цел(ДанныеПараметров["top"])));
	Скрипт = СтрЗаменить(Скрипт, "%6", XMLСтрока(Цел(ДанныеПараметров["width"])));
	Скрипт = СтрЗаменить(Скрипт, "%7", XMLСтрока(Цел(ДанныеПараметров["height"])));
...
Функция ЯПоказываюСериюПодсказокEnjoyHintВБраузере(ТабПарам1 = Неопределено, ТабПарам2 = Неопределено, 
	...
	Скрипт = 
		...
		|$('.enjoyhint_close_btn').hide();
		|
		|setTimeout(() => { commonParent.remove(); }, " + XMLСтрока(ОбщееВремя) + ");";
...
&НаКлиенте
Процедура ВременноОтключитьБраузерОтТестМенеджера(ДлительностьОтключения)
...
Процедура ЯНажимаюНаПодменюКоманднойПанелиФормыВБраузере(ИмяПодменю,ИскатьПоИмени = Ложь) Экспорт
	ТекстОшибки = "";
	БылаОшибка = Ложь;
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Попытка
			...
			Если ПредыдущийЭлементФормы <> ТекущийЭлементФормы Тогда
				//значит получилось активизировать
				Ванесса.СделатьДействияПриЗаписиВидео();
				Ванесса.СделатьДействияПриЗаписиHTML();
			КонецЕсли;	 
...
Процедура ЯНажимаюНаПодменюСИменемКоманднойПанелиФормыВБраузере(ИмяПодменю) Экспорт
