...
Процедура ТекущееОкноОжидаемомуОкнуТогда(Знач ВидСравнения,Знач ИмяОкна) Экспорт
	...
	Попытка
		...
		Если Контекст.Свойство(ИмяОкна) Тогда
...
Процедура ВТаблицеПолеИмеетЗначениеТогда(ИмяТЧ,ИмяПоля,ОжидаемоеЗначение,ИскатьПоИмени = Ложь) Экспорт
	ТЧ = Ванесса.НайтиТЧПоИмени(ИмяТЧ);
	...
	ПредставлениеДанных = ТЧ.ПолучитьТекстЯчейки(ИмяПоля);
	Ванесса.УстановитьРезультатУсловия(НРег(ПредставлениеДанных) = НРег(ОжидаемоеЗначение));
КонецПроцедуры
