...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЯЗакрываюСеансTESTCLIENT()","ЯЗакрываюСеансTESTCLIENT","И я закрываю сеанс TESTCLIENT","Закрывает активный сеанс клиента тестирования, если он есть. При этом происходит попытка закрыть модальные диалоги, если они появляются.","Подключение TestClient.Работа с подключенными TestClient.Закрыть сеанс TestClient.Устаревшее");
...
&НаКлиенте
Процедура ОбработчикЯПодключаюКлиентТестированияИзТаблицыКлиентовТестирования() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 

&НаКлиенте
Процедура ЯПодключаюКлиентТестированияИзТаблицыКлиентовТестированияОсновной(ИмяКлиентаТестирования, ЗапускатьКлиентТестирования = Истина)
...
&НаКлиенте
Процедура ОбработчикЯПодключаюКлиентТестированияИзТаблицыКлиентовТестированияОтИмениСПаролем() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯПодключаюКлиентТестированияСПараметрами() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯПодключаюУжеЗапущенныйКлиентТестирования() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯПодключаюУжеЗапущенныйКлиентТестированияСПараметрами() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
Функция ЯАктивизируюОкноТекущегоКлиентаТестирования() Экспорт
	...
	Если ДанныеПодключения <> Неопределено
		И Ванесса.Объект.ИспользоватьКомпонентуVanessaExt
		...
		Ванесса.ВнешняяКомпонентаДляСкриншотов.НачатьВызовНайтиКлиентТестирования(ОписаниеОповещения,
			ДанныеПодключения.ПортЗапускаТестКлиента);
...
Процедура ЯАктивизируюTestClient(ИмяTestClient, НадоАктивизироватьОкно = Истина) Экспорт
	Ванесса.ОбработатьСменуАктивногоОкна();
...
Процедура ПолученКлиентТестирования(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	СтруктураСвойствПроцесса = Ванесса.ПрочитатьСтрокуJSON(РезультатВызова);
	Если СтруктураСвойствПроцесса <> Неопределено Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ОбработкаПослеНачатьВызовАктивироватьОкно", ЭтаФорма);
		Ванесса.ВнешняяКомпонентаДляСкриншотов.НачатьВызовАктивироватьОкно(Оповещение,
			ДескрипторОсновногоОкнаТекущегоКлиентаТестирования);
	Иначе	
		Ванесса.ПродолжитьВыполнениеШагов();
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ОбработкаПослеНачатьВызовАктивироватьОкно(Парам1, Парам2, Парам3) Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры
...
&НаКлиенте
Процедура ОбработчикЯПодключаюTestClientЛогинПароль() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯОткрылНовыйСеансTestClientИлиПодключилУжеСуществующий() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯОткрылНовыйСеансTestClientКакTestManagerИлиПодключилУжеСуществующийБезЗагрузкиНастроек() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯОткрылНовыйСеансTestClientИлиПодключилУжеСуществующийБезЗагрузкиНастроек() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯОткрылСеансTestClientОтИмениСПаролемИлиПодключаюУжеСуществующий() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
&НаКлиенте
Процедура ОбработчикЯПодключаюПрофильTestClient() Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры 
...
Функция ЯЧитаюДанныеТекущегоПрофиляКлиентаТестирования() Экспорт
	...
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("ИмяКлиентаТестирования",ТекущиеДанные.Имя);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("СинонимКлиентаТестирования",ТекущиеДанные.Синоним);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("ТипКлиентаТестирования",ТекущиеДанные.ТипКлиента);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("ПутьКИнфобазеКлиентаТестирования",ТекущиеДанные.ПутьКИнфобазе);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("ПортЗапускаКлиентаТестирования",ТекущиеДанные.ПортЗапускаТестКлиента);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("ДопПараметрыКлиентаТестирования",ТекущиеДанные.ДопПараметры);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("ИмяКомпьютераКлиентаТестирования",ТекущиеДанные.ИмяКомпьютера);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("PIDКлиентаТестирования",ТекущиеДанные.PIDКлиентаТестирования);	
	Ванесса.СохранитьЗначениеПеременнойВКонтекст("ДескрипторОкнаКлиентаТестирования",ТекущиеДанные.ДескрипторОкна);	
