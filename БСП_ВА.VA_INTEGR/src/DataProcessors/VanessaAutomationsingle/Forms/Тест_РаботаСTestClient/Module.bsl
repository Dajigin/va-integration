...
Процедура ЯПодключаюКлиентТестированияСПараметрамиОдинНаИнформационнуюБазу(ТабПарам) Экспорт
	Если ТабПарам.Количество() <> 2 Тогда
		ВызватьИсключение Ванесса.ПолучитьТекстСообщенияПользователю("Должно быть передано 2 строки");
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработкаПослеНачатьВызовАктивироватьПроцесс(Парам1, Парам2, Парам3) Экспорт
	Ванесса.ПродолжитьВыполнениеШагов();
КонецПроцедуры
...
&НаКлиенте
Процедура ЯПодключаюПрофильTestClientОдинНаИнформационнуюБазу(ИмяПрофиля) Экспорт
	Ванесса.ОбработатьСменуАктивногоОкна();
...
&НаКлиенте
Процедура ЗакрытьВсеСеансыИнформационнойБазы(ИмяПрофиля, ПутьКИнфобазе)
