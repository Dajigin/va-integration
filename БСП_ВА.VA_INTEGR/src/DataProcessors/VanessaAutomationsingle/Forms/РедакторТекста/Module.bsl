...
Процедура ОтобразитьДанныеВиджетов() Экспорт
	...
	ОбновитьВставкиКодаVanessaEditor(СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей);
	Ванесса.РаскраситьСтрокиVanessaEditor(СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей);
КонецПроцедуры 
...
Процедура СкопироватьТекстОшибкиВБуферОбменаVanessaEditor(ДанныеОшибки) Экспорт
	...
	Если БуферОбмена = Неопределено Тогда
		ВызватьИсключение Ванесса.ПодставитьПараметрыВСтроку(
			Ванесса.Локализовать("Необходимо выключить использование компоненты <VanessaExt>.")
		);
	КонецЕсли;	 
	...
	Если Ванесса.ФайлСуществуетКомандаСистемы(Ванесса.Объект.КаталогФич) Тогда
		Если НЕ Ванесса.ЭтоКаталогКомандаСистемы(Ванесса.Объект.КаталогФич) Тогда
			Попытка
				...
				Если ДополнительныеПараметры.НадоОбновитьВставкиVanessaEditor ИЛИ НадоУстановитьТекст Тогда
...
Процедура РаскраситьСтрокиРедактора()
	СтрокиПодсценариевВиджетов = Новый Массив;
	СтрокиНеявныхОбластей = Новый Массив;
	РаскраситьСтрокиРедактораРекурсивно(Ванесса.Объект.ДеревоТестов, СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей);
	Ванесса.УстановитьДанныеДляРаскраскиСтрок(СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей);
КонецПроцедуры 

&НаКлиенте
Процедура ОбновитьВставкиКодаVanessaEditor(СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей)
	...
	ОбновитьВставкиКодаVanessaEditorРекурсивно(Ванесса.Объект.ДеревоТестов, 0, СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей);
КонецПроцедуры 
...
&НаКлиенте
Процедура РаскраситьСтрокиРедактораРекурсивно(СтрокаДерева, СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей)
	Для Каждого ТекСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		Если ТекСтрока.Тип = "ШагСценарий" Тогда
			Ванесса.VanessaEditor.setLineStyle(ТекСтрока.НомерСтрокиВФиче, "", Ложь, Ложь, Истина);
			СтрокиПодсценариевВиджетов.Добавить(Новый Структура("НомерСтроки,ИдВиджета", ТекСтрока.НомерСтрокиВФиче, ""));
			Продолжить;
		ИначеЕсли ТекСтрока.Тип = "Область" Тогда
			Если Лев(СокрЛ(ТекСтрока.Имя), 1) <> "*" И ПустаяСтрока(ТекСтрока.ДопТип) Тогда
				Ванесса.VanessaEditor.setLineStyle(ТекСтрока.НомерСтрокиВФиче, "", Истина, Ложь, Ложь);
				СтрокиНеявныхОбластей.Добавить(Новый Структура("НомерСтроки,ИдВиджета", ТекСтрока.НомерСтрокиВФиче, ""));
			КонецЕсли;	 
		КонецЕсли;	 
		
		РаскраситьСтрокиРедактораРекурсивно(ТекСтрока, СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей);
	КонецЦикла;	 
КонецПроцедуры 

&НаКлиенте
Процедура ОбновитьВставкиКодаVanessaEditorРекурсивно(СтрокаДерева, ТекУровень, СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей)
	Для Каждого ТекСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		Если ТекСтрока.Тип = "ШагСценарий" Тогда
			СтрокиПодсценариевВиджетов.Добавить(Новый Структура("НомерСтроки,ИдВиджета", ТекСтрока.НомерСтрокиВФиче, ""));
			СделатьВставкуВиджетаСПодчиненнымиСтрокамиVanessaEditor(ТекСтрока, ТекУровень, СтрокиПодсценариевВиджетов);
			Продолжить;
		...
		ОбновитьВставкиКодаVanessaEditorРекурсивно(ТекСтрока, ТекУровень + 1, СтрокиПодсценариевВиджетов, СтрокиНеявныхОбластей);
	КонецЦикла;	 
КонецПроцедуры 
