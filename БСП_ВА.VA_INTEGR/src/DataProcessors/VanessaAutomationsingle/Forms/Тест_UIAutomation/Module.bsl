...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		...
		"ЯЗапоминаюРодителяЭлементаСТипомПроцессаВПеременнуюUIAutomation","И я запоминаю родителя элемента ""Заголовок/ID"" с типом ""Тип"" процесса ""PID"" в переменную ""ИмяПеременной"" UI Automation",
...
&НаКлиенте
Процедура ПустоеОповещениеСПродолжениемВыполненияСценария(Парам1, Парам2, ДопПараметры) Экспорт
...
&НаКлиенте
Функция ОставитьТолькоТекстовыеЭлементыЕслиЭтоВозмжоно(ДанныеПолей)
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Попытка
			...
			Для Каждого СтрокаТаблицаЭлементов Из ТаблицаЭлементов Цикл
				...
				ДанныеПолей = ОставитьТолькоТекстовыеЭлементыЕслиЭтоВозмжоно(ДанныеПолей);
...
Функция ПолучитьКорректныйPID(PID)
	...
	Если НРег(PID) <> НРег("ЭтотСеанс") И НРег(PID) <> НРег("ЭтотПроцесс") Тогда
		Возврат Число(PID); 
	КонецЕсли;	 
...
//@ЯРисуюСтрелкуКЭлементамФормыUIAutomation(Парам01,ТабПарам1,ТабПарам2)
Функция ЯДелаюЭффектЗатемненияУЭлементовФормыUIAutomation(Знач PID, ТаблицаЭлементов, ПараметрыОтрисовки = Неопределено, ОбработчикПродолжения = Неопределено) Экспорт
...
&НаКлиенте
Процедура НарисоватТеньПоКоординатам(Left, Top, Right, Bottom, ПараметрыОтрисовки, ОбработчикПродолжения = Неопределено)
	Ванесса.ЗапретитьВыполнениеШагов();
...
Функция ОпределитьЭлементUIAutomationЭлементаФормыVAПоЕгоИмени(PID, ИмяЭлемента, ТипЭлемента, ВсеПоля, ИмяЭлементаСТемжеРодителем, РодителиЭлементов)
	...
	Если ЗначениеЗаполнено(ЗаголовокЭлемента) Тогда
		...
		Если ТипЗнч(ЭлементФормы) = Тип("ГруппаФормы") И ДанныеПолей.Количество() > 1 Тогда
...
&НаКлиенте
Процедура ЭлементыГруппыРекурсивно(Группа, Результат)
...
//@ЯДелаюКликПоЭлементуФормыVAUIAutomationUIAutomation(Парам01,Парам02)
Функция ЯДелаюКликПоЭлементуФормыVAUIAutomationUIAutomation(PID, ИмяЭлемента) Экспорт
	...
	Ванесса.КликПоЭлементуБезПеремещенияМышкиUIAutomation(Элем.size);
...
&НаКлиенте
Процедура СлужебныйОбработчикОжидания()
...
&НаКлиенте
Процедура ОпределитьКоординатыТекстаВРедактореВДиапазонеСтрок(Left, Top, Bottom, Right, НачНомерСтроки, КонНомерСтроки);
...
Функция ЯДелаюПодсветкуФрагментаТекстаВРедактореVAВРежимеОбучения(ТекстПоиска, ТекстПодсказки, ПереданныйНомерСтроки = Неопределено) Экспорт
	...
	Если НЕ НашлиТекстПоиска Тогда
		ВызватьИсключение Ванесса.ПодставитьПараметрыВСтроку(Ванесса.Локализовать("Не получилось найти текст <%1> в редакторе."), ТекстПоиска);
	КонецЕсли;	 
...
Функция ЯОжидаюНажатиеНаЭлементСИменемИТипомПоказываяТекстПокаНеПоявитсяЭлементСИменемИТипом(ИмяЭлементаПодсветки,ТипЭлементаПодсветки,ТекстСообщения,ИмяЭлементаОжидания,ТипЭлементаОжидания) Экспорт
	...
	ЯДелаюЭффектЗатемненияУЭлементовФормыUIAutomation(PID, ЭлементаПодсветки, ЭлементаПодсветкиПараметрыОтрисовки, "ОтрисовкаРамкиПослеЗатемненияЭлемента");
	

КонецФункции   

&НаКлиенте
Процедура ОтрисовкаРамкиПослеЗатемненияЭлемента()             
	Попытка
...
&НаКлиенте
Процедура ПослеОтрисовкиРамкиВокругЗатемненногоЭлемента() 
...
&НаКлиенте
Procedure ПроверяемНаличиеПоляПоИмениИСодержанию()
