...
&НаСервере
Функция ЗначениеКонстанты(ИмяКонстанты, ИзвлекатьИзХранилища, КакАдресВХ)
...
Функция СостояниеВанессы()
	Возврат Ванесса.ПолучитьСостояниеVanessaBehavior();	
КонецФункции 

&НаКлиенте
Функция ИмяТекущегоСценария()
	Возврат СостояниеВанессы().ТекущийСценарий.Имя;	
КонецФункции
...
Процедура ОчиститьПеременные(ИменаПеременных, КэшКонстант = Неопределено)
	...
	Если ЗначениеЗаполнено(КэшКонстант) Тогда		
...
Процедура ОткатитьИзмененияБазыНаСервере(АдресПараметров)	
	...
	Если ПараметрыОтката.Свойство("КэшКонстант") Тогда
		Для каждого КлючИЗначение Из ПараметрыОтката.КэшКонстант Цикл
			ИмяКонстанты = КлючИЗначение.Ключ;
	...
	Для каждого УдаляемаяСсылка Из ПараметрыОтката.Ссылки Цикл
...
Процедура СохранитьЗначениеКонстанты(ИмяКонстанты, ПомешатьВХранилище, УстанавливатьПараметрСеанса)
	...
	ОписаниеЗначения.Адрес = ЗначениеКонстанты(ИмяКонстанты, ПомешатьВХранилище, Истина);
...
Процедура ПроверитьУстановитьСсылкуПД(ТипИлиОписаниеТипов, ЗначениеРеквизита, ШаблонИсключения)
	...
	ПроверитьИдентификатор(ЗначениеРеквизита, ШаблонИсключения);	
...
Функция СсылкаПредопределенного(Тип, Имя)
	...
	Иначе
		...
		Исключение
		    ВызватьИсключение "Предопределенный элемент не существует: " + ИмяПредопределенного;
		КонецПопытки;		
	КонецЕсли; 
