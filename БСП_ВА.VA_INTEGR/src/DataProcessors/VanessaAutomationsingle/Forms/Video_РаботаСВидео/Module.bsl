...
Функция СоздатьКартинкуИзТекста(ПараметрыВидео,Знач Текст,ТипВыводаТекста,pointsize)
	...
	Если Не Ванесса.ФайлСуществуетКомандаСистемы(КартинкаТекст) Тогда
		ТекстСообщения = Ванесса.ПолучитьТекстСообщенияПользователю("Команда по созданию картинки из текста не выполнена: %1");
	...
	Для каждого Сценарий Из МассивСценариевДляВыполнения Цикл
		...
		Для каждого Шаг Из Шаги Цикл
			...
			Иначе
				...
				Страница.ЗаписатьСтроку("  <video width=""1280"" height=""720"" loop onmouseover=""this.play()"" onmouseout=""this.pause()"" onclick=""fullscreen(this)"">");
...
Функция ПолучитьФайлMP3ИзТекста(ПараметрыВидео,Текст,Знач ИмяФайла)
	...
	Иначе	
		...
		ТекстДляMp3 = ЧтениеТекста.Прочитать();
		ЧтениеТекста.Закрыть();
		ИмяMP3 = ПолучитьMP3ИзТекстаAmazonPoly(ПараметрыВидео,ТекстДляMp3,ИмяФайла);
	КонецЕсли;	 
	
	Если ПараметрыВидео.ЗаписьВидеоКэшироватьФайлыTTS Тогда
		ДобавитьMp3ШагаВКэш(ИмяMP3,ТекстДляMp3,ПараметрыВидео);
	КонецЕсли;	 
...
&НаКлиенте
Функция ПолучитьMP3ИзТекстаAmazonPoly(ПараметрыВидео,Текст,ИмяФайлаШага)
	...
	СтруктураПараметровЗапроса.Вставить("LanguageCode", ПараметрыВидео.ЗаписьВидеоAmazonЯзык);
	...
	СтруктураПараметровЗапроса.Вставить("VoiceId", ПараметрыВидео.ЗаписьВидеоAmazonГолос);
	...
	Если Ответ.StatusCode = 200 Тогда
		...
		Иначе 
			ТекстИсключения = Ванесса.ПолучитьТекстСообщенияПользователю("Не удалось получить данные. Код результата: <%1>. Ответ: <%2>.");
...
Функция КаталогГдеЛежитКэш(ПараметрыВидео,БазовыйКаталог)
	...
	Иначе	
		//Amazon
		КаталогДанногоГолоса = БазовыйКаталог + "\" + ПараметрыВидео.ЗаписьВидеоAmazonГолос;
		Если НЕ Ванесса.ФайлСуществуетКомандаСистемы(КаталогДанногоГолоса) Тогда
			Ванесса.СоздатьКаталогКомандаСистемы(КаталогДанногоГолоса);
		КонецЕсли;	 
