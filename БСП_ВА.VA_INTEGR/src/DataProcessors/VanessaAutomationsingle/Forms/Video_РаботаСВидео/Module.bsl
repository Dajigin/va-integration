...
Функция ПолучитьФайлMP3ИзТекста(ПараметрыВидео, Текст, Знач ИмяФайла, ПараметрыГолоса = Неопределено)
	Если НЕ ПараметрыВидео.ЗаписьВидеоДелатьНаложениеTTS И НЕ ПараметрыВидео.ОзвучкаВыполненияСценария Тогда
		Возврат Неопределено;
	КонецЕсли;	 
	...
	Для Сч = 1 По 3 Цикл
		...
		Попытка 
			...
			ИначеЕсли ПараметрыГолоса.ТипОзвучки = 3 Тогда
				// Озвучка от Сбер
...
Функция ПолучитьПараметрыГолосаПоУмолчанию(ПараметрыВидео)
	...
	Если ПараметрыВидео.ЗаписьВидеоТипОзвучкиTTS = 1 Тогда
		...
		ПараметрыГолоса.Вставить("Скорость", ПараметрыВидео.ЗаписьВидеоYandexTTSСкорость);
	...
	ИначеЕсли ПараметрыВидео.ЗаписьВидеоТипОзвучкиTTS = 2 Тогда
		...
		ПараметрыГолоса.Вставить("Эмоция", ПараметрыВидео.ЗаписьВидеоYandexTTSЭмоция);
		ПараметрыГолоса.Вставить("Скорость", 1);
		ПараметрыГолоса.Вставить("Язык", ПараметрыВидео.ЗаписьВидеоAmazonЯзык);
		
	ИначеЕсли ПараметрыВидео.ЗаписьВидеоТипОзвучкиTTS = 3 Тогда
		
		ПараметрыГолоса.Вставить("ТипОзвучки", 3);
		ПараметрыГолоса.Вставить("Префикс", "s_");
		ПараметрыГолоса.Вставить("Голос", ПараметрыВидео.ЗаписьВидеоСберTTSГолос);
		
	Иначе	
		
		ПараметрыГолоса.Вставить("ТипОзвучки", -1);
		ПараметрыГолоса.Вставить("Префикс", "");
		ПараметрыГолоса.Вставить("Голос", Неопределено);
		ПараметрыГолоса.Вставить("Эмоция", Неопределено);
		ПараметрыГолоса.Вставить("Скорость", Неопределено);
		ПараметрыГолоса.Вставить("Язык", Неопределено);
...
Процедура ЗавершитьЗаписьВыполненияТестов(ПараметрыВидео) Экспорт
	...
	ИмяФайлаВидеоИсходный = Ванесса.ДополнитьСлешВПуть(ПараметрыВидео.КаталогДляЗаписиВидеоВыполненияТестов)
		+ "scen" + XMLСтрока(ПараметрыВидео.ТекИД_СценарияВМассиве) + ".mp4";
	...
	ИмяФайлаВидеоРезультат = ПараметрыВидео.КаталогДляЗаписиВидеоВыполненияТестов
		+ "\" + XMLСтрока(ТекущаяУниверсальнаяДатаВМиллисекундах()) + "_"
