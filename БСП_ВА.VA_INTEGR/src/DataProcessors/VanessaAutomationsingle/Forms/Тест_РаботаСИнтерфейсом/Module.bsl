...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЭлементФормыСталРавен(Парам01,Парам02)","ЭлементФормыСталРавен","Тогда элемент формы ""Заголовок поля"" стал равен ""ЗначениеПоля""","Проверка значения элемента формы","UI.Формы.Поля.Проверка значения поля");
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ЭлементФормыСИменемСталРавен(Парам01,Парам02)","ЭлементФормыСИменемСталРавен","Тогда элемент формы с именем ""ИмяПоля"" стал равен ""ЗначениеПоля""","Проверка значения элемента формы","UI.Формы.Поля.Проверка значения поля");
...
Процедура ВОткрытойФормеВТЧВПолеСЗаголовкомЯВвожуТекст(ИмяТЧ,ИмяРеквизита,Знач Текст,ИскатьПоИмени = Ложь) Экспорт
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Попытка
			...
			Ванесса.ВвестиВПолеТекст(ПолеРеквизит, Текст, Истина);
			Попытка
				Если Текст = "" Тогда
	...
	Для СчетчикПопыток = 1 По КоличествоПопыток Цикл
		...
		Попытка
			Если СчетчикПопыток = 1 И Ванесса.ЗначениеЗафиксированногоКонтейнера() <> Неопределено Тогда
				ТЧ = НайтиТЧПоИмени(ИмяТЧ, Ванесса.ЗначениеЗафиксированногоКонтейнера());
			Иначе	
				ТЧ = НайтиТЧПоИмени(ИмяТЧ);
			КонецЕсли;	 
...
Процедура ВОткрытойФормеЯПерехожуКЗакладкеСЗаголовком(ИмяЗакладки,ИскатьПоИмени = Ложь) Экспорт
	//ГруппаСтраницы = Ванесса.НайтиРеквизитОткрытойФормыПоЗаголовку("ГруппаСтраницы", ИскатьПоИмени, Ложь, Тип("ТестируемаяГруппаФормы"));
	...
	//	ДопПараметры.Вставить("КонтейнерДляПоиска", ГруппаСтраницы);
	//КонецЕсли;	 
...
Функция ЯЗапоминаюСодержимоеФайлаВПеременную(ИмяФайла,ИмяПеременной) Экспорт
	ПолноеИмяФайла = Ванесса.НайтиФайлСвязанныйСТестами(ИмяФайла);
	Если ПолноеИмяФайла = Неопределено Тогда
		ТекстИсключения = "Файл <%1> не найден.";
		ТекстИсключения = СтрЗаменить(ТекстИсключения,"%1",ИмяФайла);
...
Процедура УстановитьЗначениеВПолеПриЗаполненииТаблицы(Таблица,РеквизитТаблицы,Текст,ИмяТаблицы,ВводитьТекст)
	...
	Попытка
		...
		Если ВыпадающийСписокОткрыт И Ванесса.Версия8313ИлиВыше Тогда
			...
			ПредставлениеСпискаВыбора = РеквизитТаблицы.ПолучитьПредставлениеСпискаВыбора();
			Ном = 0;
