...
#Область ОбработчикиСобытийТаблицыФормыНаборыТестов
&НаКлиенте
...
//			КонецЕсли;
//			
//			ШаблонПутиРезультирующегоФайла = ИмяКаталогаРезультата + "\" 
...
//				ПутьКРезультирующемуФайлу = ШаблонПутиРезультирующегоФайла;
//				НомерПользователя = Формат(СтрокаПользователя.НомерСтроки, "ЧЦ=2; ЧРГ=; ЧВН=; ЧГ=;");
//				ПутьКРезультирующемуФайлу = СтрЗаменить(ПутьКРезультирующемуФайлу, "&НомерПользователя", НомерПользователя);
//				КопироватьФайл(ПутьКИсходномуФайлу, ПутьКРезультирующемуФайлу);
//				УстановитьПользователяВФайле(ПутьКРезультирующемуФайлу, СтрокаПользователя.ИмяПользователя, СтрокаПользователя.Пароль);
//			КонецЦикла;
//		КонецЦикла;
//	
...
Функция ПодготовитьТестыНабора(ОписаниеНабора, ОписаниеФайловТестов, ПутьРезультата)
	...
	ВсеФайлы = ОписаниеФайловТестов.ВсеФайлы; 
	Если ОписаниеНабора.ЭтоТаблицаИсключаемыхТегов Тогда
		ТестыДляВыполнения = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ВсеФайлы);
		Для Каждого ИмяТэга Из ОписаниеНабора.Тэги Цикл
			ФайлыТэга = ОписаниеФайловТестов.ФайлыПоТэгам.Получить(НРег(ИмяТэга));
...
Функция ЗагрузитьНастройкиНабораИзФайла(ПутьКФайлу, ОбновитьНастройкиВФорме = Истина)
	...
	Если ОбновитьНастройкиВФорме Тогда
		...
		ФайлНастроекНабора = ПутьКФайлу;
	КонецЕсли;
	Результат = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(НастройкиНабораТестов);
	Возврат Результат;
КонецФункции
