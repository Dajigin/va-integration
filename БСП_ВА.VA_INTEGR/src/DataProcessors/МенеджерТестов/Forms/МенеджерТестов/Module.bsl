...
Процедура ПодготовитьИВыполнить(Команда)
	Если СформироватьФайлыТестовДляВыполнения() Тогда
		...
		ФормаОбработкиТестирования = ОткрытьФорму("Обработка.VanessaAutomationsingle.Форма.УправляемаяФорма", 
												  ПараметрыФормы);
		ФормаОбработкиТестирования.Объект.КаталогФич = КаталогТестов;
