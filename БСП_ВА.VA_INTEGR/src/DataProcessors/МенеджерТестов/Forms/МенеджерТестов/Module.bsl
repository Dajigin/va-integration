...
// После закрытия формы выбора каталога.
// 
// Параметры:
//  РезультатЗакрытия - РежимДиалогаВопрос - Результат закрытия
//  ДопПараметр - Произвольный - Доп. параметр вызова
&НаКлиенте
	...
	Попытка
		...
		Если глоФормаОбработкиВА.ФормаVanessaAutomationОткрылась Или глоКоличествоПроверокОткрытияФормыВА
			...
			Если глоФормаОбработкиВА.ФормаVanessaAutomationОткрылась Тогда
				...
				глоФормаОбработкиВА.Объект.ИспользоватьКомпонентуVanessaExt = Ложь;
//				глоФормаОбработкиВА.ВыполнитьПодключениеВнешнейКомпоненты();
				глоФормаОбработкиВА.Объект.ИспользоватьВнешнююКомпонентуДляСкриншотов = Ложь;
				глоФормаОбработкиВА.Объект.ИспользоватьПарсерGherkinИзКомпонентыVanessaExt = Ложь;
				...
				ВАИНТ_ТестированиеКлиент.ЗаполнитьБиблиотекиШагов(глоФормаОбработкиВА);
				ВАИНТ_ТестированиеКлиент.ДополнитьПараметрыЗапускаКлиентов(глоФормаОбработкиВА);
