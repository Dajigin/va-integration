...
// В процессе запуска обработки ВА в этой переменной хранится ссылка на открываемую форму обработки.	
&НаКлиенте
Перем ФормаОбработкиВА;
// При открытии формы ВА в этой переменной хранится количество выполнения процедуры обработчика ожидания открытия формы.
&НаКлиенте
Перем КоличествоПроверокОткрытияФормыВА;
#КонецОбласти
...
Процедура ПодготовитьИВыполнить(Команда)
	ФормаОбработкиВА = Неопределено;
	Если СформироватьФайлыТестовДляВыполнения() Тогда
	
		КаталогТестов = ПутьККаталогуПодготовленныхТестов();
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("КаталогПроекта", КаталогТестов);

		КоличествоПроверокОткрытияФормыВА = 0;
		ФормаОбработкиВА = ОткрытьФорму("Обработка.VanessaAutomationsingle.Форма.УправляемаяФорма", 
												  ПараметрыФормы);
...
Процедура ОжиданиеОткрытиеОбработкиВА() Экспорт
	Попытка
		...
		Если ФормаОбработкиВА.ФормаVanessaAutomationОткрылась
			Или КоличествоПроверокОткрытияФормыВА = МаксимальноеКоличествоПроверокОткрытияФормы Тогда
	
			ОтключитьОбработчикОжидания("ОжиданиеОткрытиеОбработкиВА");
			Если ФормаОбработкиВА.ФормаVanessaAutomationОткрылась Тогда
				ДопПараметры = Новый Структура();
				ФормаОбработкиВА.Объект.КаталогФич = КаталогТестов;
				ФормаОбработкиВА.Объект.ДелатьОтчетВФорматеАллюр = Истина;
				ФормаОбработкиВА.Объект.ДелатьОтчетВФорматеCucumberJson = Истина;
				ФормаОбработкиВА.Объект.ДелатьОтчетВФорматеjUnit = Истина;
				ФормаОбработкиВА.Объект.КаталогВыгрузкиAllure = ПутьККаталогуЛогов_Allure();
				ФормаОбработкиВА.Объект.КаталогВыгрузкиCucumberJson = ПутьККаталогуЛогов_Cucumber();
				ФормаОбработкиВА.Объект.КаталогВыгрузкиJUnit = ПутьККаталогуЛогов_JUnit();
					
				ДопПараметры.Вставить("ПерезагрузитьИВыполнить", Истина);
				ФормаОбработкиВА.ЗагрузитьФичи(ДопПараметры);
...
Процедура ОжиданиеЗавершенияТестов() Экспорт
	КаталогЛогов = ПутьККаталогуЛогов_Allure();
	МассивФайлов = НайтиФайлы(КаталогЛогов, "*.json");
	Если МассивФайлов.Количество() <> 0 Тогда
		ОтключитьОбработчикОжидания("ОжиданиеЗавершенияТестов");
		ПутьКИсполняемомуФайлуAllure = ПутьККаталогуДистрибутиваAllure() + "\bin\allure.bat";		
...
Процедура ОткрытьКаталогПодготовленныхТестов(Команда)
	ЗапуститьПриложение(ПутьККаталогуПодготовленныхТестов());
КонецПроцедуры
...
Функция СформироватьФайлыТестовДляВыполнения()
	...
	ИмяКаталогаРезультата = ПутьККаталогуПодготовленныхТестов();
	...
	СоздатьКаталог(ПутьККаталогуЛогов());
	СоздатьКаталог(ПутьККаталогуЛогов_Allure());
	СоздатьКаталог(ПутьККаталогуЛогов_Cucumber());
	СоздатьКаталог(ПутьККаталогуЛогов_JUnit());
	
	СоздатьКаталог(ПутьККаталогуДистрибутиваAllure());
...
КонецФункции
 ...
 Процедура РаспаковатьДистрибутивAllure()
 	ПутьККаталогуДистрибутива = ПутьККаталогуДистрибутиваAllure();
...
&НаКлиенте
Функция ПутьККаталогуПодготовленныхТестов()
	Возврат БазовыйКаталогТестов + "\Temp\ТестыДляВыполнения";
КонецФункции

&НаКлиенте
Функция ПутьККаталогуЛогов()
	Возврат ПутьККаталогуПодготовленныхТестов() + "\Logs";
КонецФункции

&НаКлиенте
Функция ПутьККаталогуДистрибутиваAllure()
	Возврат ПутьККаталогуПодготовленныхТестов() + "\Allure_Distrib";
КонецФункции

&НаКлиенте
Функция ПутьККаталогуЛогов_Allure()
	Возврат ПутьККаталогуЛогов() + "\Allure";
КонецФункции

&НаКлиенте
Функция ПутьККаталогуЛогов_Cucumber()
	Возврат ПутьККаталогуЛогов() + "\Cucumber";
КонецФункции

&НаКлиенте
Функция ПутьККаталогуЛогов_JUnit()
	Возврат ПутьККаталогуЛогов() + "\JUnit";
КонецФункции

&НаКлиенте
Функция ПутьККаталогуНастроекНаборов()
...
Процедура СохранитьКакНастройкиНабора()
	ОчиститьСообщения();
	ПутьККаталогуНабора = ПутьККаталогуНастроекНаборов();
